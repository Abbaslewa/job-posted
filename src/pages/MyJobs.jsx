...
const MyJob = () => {
  ...
  useEffect(() => {
    setIsLoading(true);
    fetch(`http://localhost:5000/myJobs/abbasomerhassn@gmail.com`)
      .then(res => res.json()) 
      .then(data => {
        setJobs(data);
      ...
      <div className='my-jobs-container'>
        ...
        <div className='search-box p-2 text-center mb-2'>
          <input 
            ...
            className='py-2 border focus:outline-none lg:w-6/12 mb-4 w-full pl-3' 
          />
          <button 
            onClick={handleSearch} 
            className='text-white bg-blue font-semibold px-8 py-2 rounded-sm mb-1'>
            Search
          </button>
        </div>
      </div>
      ...
      <table className="items-center bg-transparent w-full border-collapse ">
        <thead>
          ...
          <th className="px-6 bg-blueGray-50 text-blueGray-500 align-middle border border-solid border-blueGray-100 py-3 text-xs uppercase border-l-0 border-r-0 whitespace-nowrap font-semibold text-left">
           ...
           <th classNameclass="px-6 bg-blueGray-50 text-blueGray-500 align-middle border border-solid border-blueGray-100 py-3 text-xs uppercase border-l-0 border-r-0 whitespace-nowrap font-semibold text-left">
                          COMPONY NAME
                        </th>
          <th className="px-6 bg-blueGray-50 text-blueGray-500 align-middle border border-solid border-blueGray-100 py-3 text-xs uppercase border-l-0 border-r-0 whitespace-nowrap font-semibold text-left">
                         SALARY
